...
@testset "Water and heat, dirichlet at top" begin
    ...
    gradf2c_heat =
        Operators.GradientF2C(bottom = Operators.SetValue(f_heat_bottom))
    ...
    If_d_heat =
        Operators.InterpolateC2F(top = Operators.SetValue(K_top * ρe_int_l_top))


    gradc2f_water = Operators.GradientC2F(top = Operators.SetValue(h_top))# for ∇h on face

    dI_orig = @. gradf2c_heat(
        If_heat(κ) * gradc2f_heat(T) +
        If_d_heat(ρe_int_l * K) * gradc2f_water(h),
    )
    ...
    dθ_orig =
        @. Operators.GradientF2C(bottom = Operators.SetValue(f_water_bottom))(
            If_water(K) * gradc2f_water(h),
        )
    ...
    f_heat_top =
        κ_top * (Ttop - parent(T)[end]) / (dz_top * 2) .+
        ρe_int_l_top * f_water_top

    interpc2f = Operators.InterpolateC2F()
    gradc2f_heat = Operators.GradientC2F()
    gradf2c_heat = Operators.GradientF2C(
        top = Operators.SetValue(f_heat_top),
        bottom = Operators.SetValue(f_heat_bottom),
    )
    gradc2f_water = Operators.GradientC2F()
    gradf2c_water = Operators.GradientF2C(
        top = Operators.SetValue(f_water_top),
        bottom = Operators.SetValue(f_water_bottom),
    )

    dI_flux = @. gradf2c_heat(
...
@testset "Water and heat, dirichlet at different sides" begin
    ...
    gradc2f_water =
        Operators.GradientC2F(top = Operators.SetGradient(f_water_top / K_top))

    dI_orig = @. gradf2c_heat(
        If_heat(κ) * gradc2f_heat(T) +
        If_d_heat(ρe_int_l * K) * gradc2f_water(h),
    ) # this gives correct water flux at top, and ρe_int is correct on top b/c we have T_top
    ...
    dθ_orig = @. Operators.GradientF2C(top = Operators.SetValue(f_water_top))(
        If_water(K) * gradc2f_water(h),
    )
...
@testset "Water alone, Dirichlet at top" begin
    ...
    gradf2c_new = Operators.GradientF2C(
...
@testset "Heat alone, Dirichlet at top/bottom" begin
    ...
    gradc2f = Operators.GradientC2F(
        ...
        bottom = Operators.SetValue(Tbottom),
    )
    gradf2c = Operators.GradientF2C()
    If = Operators.InterpolateC2F(
        top = Operators.Extrapolate(),
        bottom = Operators.Extrapolate(),
    )# not sure about this. look into case in integrated model.
